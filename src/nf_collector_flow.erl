...
start_link() ->
    gen_server:start_link({local, nf_collector_flow}, ?MODULE, [], []).
...
terminate(Reason, State) ->
    lager:error("nf_collector_flow terminate with reason: ~p. State: ~p~n", [Reason, State]).
